...
export class IdentityClient {
   ...
   * @deprecated Use registerWithTokenId instead if contract requires tokenId parameter
   */
  ...
  /**
   * Register a new agent with a specific tokenId (no URI)
   * Spec: function register(uint256 tokenId) returns (uint256 agentId)
   * @param tokenId - The token ID to register
   */
  async registerWithTokenId(tokenId: bigint): Promise<{ agentId: bigint; txHash: string }> {
    const result = await this.adapter.send(
      this.contractAddress,
      IdentityRegistryABI,
      'register',
   ...
   * @deprecated Use registerWithTokenIdAndURI instead if contract requires tokenId parameter
   */
  ...
  /**
   * Register a new agent with a specific tokenId and token URI
   * Spec: function register(uint256 tokenId, string memory tokenUri) returns (uint256 agentId)
   * @param tokenId - The token ID to register
   * @param tokenURI - URI pointing to agent registration file (MAY use ipfs://, https://, etc.)
   */
  async registerWithTokenIdAndURI(tokenId: bigint, tokenURI: string): Promise<{ agentId: bigint; txHash: string }> {
    const result = await this.adapter.send(
      this.contractAddress,
      IdentityRegistryABI,
      'register',
   ...
   * @deprecated Use registerWithTokenIdAndMetadata instead if contract requires tokenId parameter
   */
  ...
  /**
   * Register a new agent with a specific tokenId, URI and optional on-chain metadata
   * Spec: function register(uint256 tokenId, string memory tokenUri, MetadataEntry[] memory metadata) returns (uint256 agentId)
   * @param tokenId - The token ID to register
   * @param tokenURI - URI pointing to agent registration file
   * @param metadata - OPTIONAL on-chain metadata entries
   */
  async registerWithTokenIdAndMetadata(
    tokenId: bigint,
    tokenURI: string,
    metadata: MetadataEntry[] = []
  ): Promise<{ agentId: bigint; txHash: string }> {
    ...
    const result = await this.adapter.send(
      ...
      IdentityRegistryABI,
      'register',
      [tokenId, tokenURI, metadataFormatted]
    );
